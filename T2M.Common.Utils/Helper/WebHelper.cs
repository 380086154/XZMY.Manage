...
namespace T2M.Common.Utils.Helper
{
    ...
    public class WebHelper
    {
        ...
        {
            HttpCookie objCookie = new HttpCookie(cookiesName.Trim()) { Expires = DateTime.Now.AddYears(-5) };
        ...
        {
            return HttpWebRequest(url, parameters, Encoding.GetEncoding("utf-8"), false);
        }
        ...
        /// <param name="parameters">提交的参数,格式：参数1=参数值1&amp;参数2=参数值2</param>
        /// <returns></returns>
        public static string HttpWebRequestPost(string url, string parameters)
        {
        ...
        /// <param name="cookie">Cookie容器</param>
        /// <param name="timeout">超时时间</param>
        public static string HttpWebRequestPost(string url, object paramData,
             string contentType = "application/x-www-form-urlencoded", CookieContainer cookie = null, int timeout = 120000)
        {
            ...
            byte[] postData = ToBytes(paramData);
            request.Method = "POST";
            request.ContentType = contentType;
            request.ContentLength = postData.Length;
            using (Stream stream = request.GetRequestStream())
