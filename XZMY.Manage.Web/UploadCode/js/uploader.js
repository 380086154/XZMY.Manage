$(function () {
    ...
    $.fn.InitUploader = function (b) {
        ...
        var fun = function (parentObj) {
            ...
            if (p.NoUrl) {
                p.sendurl += "&ImageFolder=" + b.ImageFolder;
            }
            //if (!p.multiple) {
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    ...
                    if (p.ImageFolder == "Album") {
                        //相册管理
                        parentObj.siblings(".photo-list").empty()
                        addImageNoUrl(parentObj, data.path, data.thumb);
                    }
                    else
                    {
                        ...
                        if (!p.NoUrl) {
                            ...
                            if (!p.multiple) {
                                ...
                                $(progressObj).siblings(".photo-list").empty();
...
function addImage(targetObj, originalSrc, thumbSrc) {
    //插入到相册UL里面
    var newLi = $(''
    + '<div style="float:left; width:200px; height:238px; background-color:#ffffff; margin:5px;">'
    + '    <div style="float:left; width:200px; height:200px; ">'
    + '        <div style=" width:200px; height:30px;position:absolute; text-align:right; ">'
    + '            <input type="button" value="删除"  onclick="delImg(this);" /><span data-role="remove"></span>'
    
    + '        </div>'
    + '        <img src="' + originalSrc + '" style="width:100%; height:100%;" />'
    + '    </div>'
    + '    <div style="float:left; width:200px; height:38px; ">'
    + '        <input type="text" name="hid_photo_Url" class="hid_photo_Url"  value="" style="width:185px;margin-left:1px" />'
    + '        <input type="hidden" name="hid_photo_ImageUrl" class="hid_photo_ImageUrl" value="' + originalSrc + '" />'
    + '    </div>'
    + '</div>');
    newLi.appendTo(targetObj.siblings(".photo-list"));
...
function addImageNoUrl(targetObj, originalSrc, thumbSrc) {
    //插入到相册UL里面
    var newLi = $(''
    + '<div style="float:left; width:200px; height:238px; background-color:#ffffff; margin:5px;">'
    + '    <div style="float:left; width:200px; height:200px; ">'
    ...
    + '    </div>'
    + '    <div style="float:left; width:200px; height:38px; ">'
    + '        <input type="hidden" name="hid_photo_Url" class="hid_photo_Url"  value="" style="width:185px;margin-left:1px" />'
    + '        <input type="hidden" name="hid_photo_ImageUrl" class="hid_photo_ImageUrl" value="' + originalSrc + '" />'
    + '    </div>'
    + '</div>');
    newLi.appendTo(targetObj.siblings(".photo-list"));
...
*/
var ShowImage = function (imgSrc, Url, IsShowDel, IsShowUrl) {

    //插入到相册UL里面
    var newLi = '';
    newLi += '<div style="float:left; width:200px; height:238px; background-color:#ffffff; margin:5px;">';
    newLi += '    <div style="float:left; width:200px; height:200px; ">';
    newLi += '        <div style=" width:200px; height:30px;position:absolute; text-align:right; ">';
    if (IsShowDel) {//是否显示删除按钮
        newLi += '            <input type="button" value="删除"  onclick="delImg(this);" /><span data-role="remove"></span>';
    }
    newLi += '        </div>';
    newLi += '        <img src="' + imgSrc + '" style="width:100%; height:100%;" />';
    newLi += '    </div>';
    newLi += '    <div style="float:left; width:200px; height:38px; ">';
    if (IsShowUrl) {
        newLi += '        <input type="text" name="hid_photo_Url" class="hid_photo_Url"  value="' + Url + '" style="width:185px;margin-left:1px" />';
    } else {
        newLi += '        <input type="hidden" name="hid_photo_Url" class="hid_photo_Url"  value="' + Url + '" style="width:185px;margin-left:1px" />';
    }
    newLi += '        <input type="hidden" name="hid_photo_ImageUrl" class="hid_photo_ImageUrl" value="' + imgSrc + '" />';
    newLi += '    </div>';
    newLi += '</div>';
...
///显示
function ShowImageNoUrl(originalSrc) {
