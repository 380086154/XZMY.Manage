...
namespace XZMY.Manage.Service.Handlers.Module
{
    ...
    {
        public ModuleCreateHandler(VmModuleEdit vm)
        {
            Model = vm;
        }
        ...
        public HandlerInvokeResult Invoke()
        {
            ...
            try
            {
                ...
                using (var wrapper = new SqlTransactionWrapper())
                {
                    try
                    {
                        var createservice = new BaseCreateService<Sys_Module>(datamodel);
            ...
            {
                LogHelper.LogException("ModuleCreateHandler", "创建失败", LogLevel.Error, ex);
